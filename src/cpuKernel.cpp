                      ...
                      for (int n = 0; n < input.centOrder + 1; n++)
                        {
                          for (int v = 3; v < (5+DIM); v++)
                            {
                              stencilData[stencilIdx(v,n)] = flow[bidx(v,i+dijk[0]*n-input.nguard,j+dijk[1]*n-input.nguard,k+dijk[2]*n-input.nguard,lb,input)];                              
                            }
                          // IE = P/(rho*(gamma - 1))
                      ...
                      for (int l = 1; l <= stencilWid; l++)
                        {
                          ...
                          for (int m = 0; m <= (l-1); m++)
                            {
                              C[1] += 2.0*centerCoef[l]*fg_QuadSplit(stencilData,jf-m, l,2,5+idir);
                              C[0] += 2.0*centerCoef[l]*fg_QuadSplit(stencilData,jf+m,-l,2,5+idir);

                              for (int idir_mom = 0; idir_mom < DIM; idir_mom++)
                                {
                                  M[idir_mom      ] = 2.0*centerCoef[l]*fg_CubeSplit(stencilData,jf-m, l,3,5+idir,5+idir_mom);
                                  M[idir_mom + DIM] = 2.0*centerCoef[l]*fg_CubeSplit(stencilData,jf+m,-l,3,5+idir,5+idir_mom);
                                }

                              PGRAD[1] += 2.0*centerCoef[l]*f_DivSplit(stencilData,jf-m, l,3);
                              PGRAD[0] += 2.0*centerCoef[l]*f_DivSplit(stencilData,jf+m,-l,3);
                              
                              for (int vel_comp = 0;  vel_comp < DIM; vel_comp ++)
                                {
                                  KE[1] += 2.0*centerCoef[l]*fg_QuadSplit(stencilData,jf-m, l,2,5+idir)*0.5*(stencilData[stencilIdx(5+vel_comp,jf-m)]*stencilData[stencilIdx(5+vel_comp,jf-m+l)]);
                                  KE[0] += 2.0*centerCoef[l]*fg_QuadSplit(stencilData,jf+m,-l,2,5+idir)*0.5*(stencilData[stencilIdx(5+vel_comp,jf+m)]*stencilData[stencilIdx(5+vel_comp,jf+m-l)]);
                                }
                              
                              IE[1] += 2.0*centerCoef[l]*fg_CubeSplit(stencilData,jf-m, l,3,0,5+idir);
                              IE[0] += 2.0*centerCoef[l]*fg_CubeSplit(stencilData,jf+m,-l,3,0,5+idir);

                              PDIFF[1] += 2.0*centerCoef[l]*fg_DivSplit(stencilData,jf-m, l,5+idir,3);
                              PDIFF[0] += 2.0*centerCoef[l]*fg_DivSplit(stencilData,jf-m, l,5+idir,3);                              
                            }
                        }
